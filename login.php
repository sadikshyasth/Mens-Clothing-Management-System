...
if(isset($_POST['form1'])) { 
    ...
    if(empty($_POST['cust_email']) || empty($_POST['cust_password'])) {
        ...
        $result= $login_object->getLoginDetails($cust_email);
        $total= $login_object->getRows($cust_email);
        ...
        if($total==0) {
            ...
            //using sha1 form
            if( $row_password != sha1($cust_password) ) {
                $error_message .= "Passwords do not match".'<br>';
            } else {
                if($cust_status == 0) {
                    $error_message .= "Sorry! Your account is inactive. Please contact to...".'<br>';
                } else {
